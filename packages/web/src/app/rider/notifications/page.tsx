...
const TYPE_META: Record<string, { icon: any; color: string; label: string }> = {
  ORDER_REQUEST:      { icon: Package,       color: 'bg-blue-100 text-blue-600',     label: 'New Order' },
...
export default function RiderNotificationsPage() {
  ...
  const markReadMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ['rider-unread-notifications'] });
    },
  });
        ...
        {FILTERS.map((f) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-colors ${
              filter === f ? 'bg-gray-900 text-brand-gold' : 'bg-muted text-muted-foreground hover:bg-muted/80'
            }`}
          >
      ...
      <div className="space-y-2">
        {isLoading ? (
          ...
          <div className="text-center py-16 text-muted-foreground">
            ...
            <p className="text-sm">You'll see updates about jobs, payments and more here</p>
          </div>
        ) : (
          filtered.map((n) => {
