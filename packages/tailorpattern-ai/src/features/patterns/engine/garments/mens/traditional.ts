...
export function generateTraditional(garmentType: string, input: EngineInput): EngineOutput {
  const { measurements: m, seamAllowance: sa } = input
  ...
  if (garmentType === 'mens-kaftan' || garmentType === 'mens-senator') {
    ...
    const kaftanOutline: PatternPath = {
    ...
    const kaftan: PatternPiece = {
      ...
      quantity: 2,
      mirror: true,
      outline: kaftanOutline,
      seamLine: buildSeamLine(kaftanOutline, sa),
      foldLine: {
      ...
      keyMeasurements: [`Chest: ${chest}cm`, `Length: ${length}cm`],
    }

    const neckFacingOutline: PatternPath = {
      isClosed: true, isSeamLine: false,
      points: [
        { x: 0, y: 0 },
    ...
    const neckFacing: PatternPiece = {
      ...
      quantity: 1,
      mirror: false,
      outline: neckFacingOutline,
      seamLine: buildSeamLine(neckFacingOutline, sa),
  ...
  const agbadaOuterOutline: PatternPath = {
    isClosed: true, isSeamLine: false,
    points: [
      { x: 0, y: 0 },
  ...
  const agbadaOuter: PatternPiece = {
    ...
    quantity: 2,
    mirror: true,
    outline: agbadaOuterOutline,
    seamLine: buildSeamLine(agbadaOuterOutline, sa),
    foldLine: {
      isClosed: false, isFoldLine: true,
      points: [{ x: 0, y: 0 }, { x: 0, y: length }],
    },
  ...
  const agbadaInnerOutline: PatternPath = {
    isClosed: true, isSeamLine: false,
    points: [
      { x: 0, y: 0 },
      { x: bodyWidth, y: 0 },
